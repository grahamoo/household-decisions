* This ado file is mostly based on the ado file found at https://eml.berkeley.edu/~jmccrary/DCdensity/,
* with several changes to format the graph

capture program drop DCdensity_format
program DCdensity_format, rclass
{
  version 9.0
  set more off
  pause on
  ...
  if ("`graph'"!="nograph") { 
    ...
    gr twoway (scatter `cellvalname' `cellmpname' if `cellmpname' >= -0.3 & `cellmpname' <= 0.3, msymbol(circle_hollow) mcolor(gray) xlabel(-0.3(0.1)0.3) xtitle("Margin of victory in male-female elections") ytitle("Frequency"))           ///
      (line `cellsmname' `evalname' if `evalname' < `breakpoint' & `evalname' >= -0.3 & `evalname' <= 0.3, lcolor(black) lwidth(medthick))   ///
        (line `cellsmname' `evalname' if `evalname' > `breakpoint' & `evalname' >= -0.3 & `evalname' <= 0.3, lcolor(black) lwidth(medthick))   ///
          (line `hi' `evalname' if `evalname' < `breakpoint' & `evalname' >= -0.3 & `evalname' <= 0.3, lcolor(black) lwidth(vthin))              ///
            (line `lo' `evalname' if `evalname' < `breakpoint' & `evalname' >= -0.3 & `evalname' <= 0.3, lcolor(black) lwidth(vthin))              ///
              (line `hi' `evalname' if `evalname' > `breakpoint' & `evalname' >= -0.3 & `evalname' <= 0.3, lcolor(black) lwidth(vthin))              ///
                (line `lo' `evalname' if `evalname' > `breakpoint' & `evalname' >= -0.3 & `evalname' <= 0.3, lcolor(black) lwidth(vthin)),             ///
                  xline(`breakpoint', lcolor(black)) legend(off) graphregion(color(white)) bgcolor(white) ylabel(, angle(horizontal))
    if ("`graphname'"!="") {
